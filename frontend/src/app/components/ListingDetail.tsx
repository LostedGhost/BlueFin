...
export function ListingDetail({ property, onBack, onOpenBooking, isAuthenticated }: ListingDetailProps) {
  ...
  return (
    <div className="bg-white pb-24 lg:pb-0">
      {/* Mobile top bar */}
      <div className="lg:hidden flex items-center justify-between px-4 py-3 border-b border-[#d5eef2]">
        <button onClick={onBack} className="w-9 h-9 flex items-center justify-center rounded-full border border-[#d5eef2]">
          <ChevronLeft className="w-4 h-4 text-[#0f2940]" />
        </button>
        <div className="flex items-center gap-2">
          <button className="w-9 h-9 flex items-center justify-center rounded-full border border-[#d5eef2]">
            <Share className="w-4 h-4 text-[#0f2940]" />
          </button>
          <button onClick={handleLike} className="w-9 h-9 flex items-center justify-center rounded-full border border-[#d5eef2]">
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-8">
        ...
        <div className="lg:hidden mb-4">
          ...
          <div className="flex items-center gap-3 text-sm flex-wrap">
            <div className="flex items-center gap-1">
              <Star className="w-3.5 h-3.5 fill-[#12b8c9] text-[#12b8c9]" />
              <span className="font-medium">{property.rating}</span>
              <span className="text-[#6b7280]">({property.reviews} avis)</span>
            </div>
            <span className="text-[#6b7280]">·</span>
        ...
        <div className="hidden lg:grid grid-cols-4 gap-2 mb-8 rounded-2xl overflow-hidden h-[400px]">
          ...
          {photos.slice(1, 4).map((photo, i) => (
            <div key={i} className="col-span-1 bg-[#eefbfd]">
              <img src={photo} alt={`Vue ${i+2}`} className="w-full h-full object-cover" />
            </div>
          ))}
          <div className="col-span-1 relative bg-[#eefbfd]">
        ...
        <div className="lg:hidden mb-4">
          <div className="relative rounded-2xl overflow-hidden bg-[#eefbfd]" style={{ aspectRatio: '4/3' }}>
          ...
          <div className="flex gap-2 mt-2 overflow-x-auto scrollbar-hide">
            {photos.slice(0, 5).map((photo, i) => (
              <button key={i} onClick={() => setActivePhoto(i)} className={`flex-shrink-0 w-14 h-14 rounded-lg overflow-hidden border-2 transition-colors bg-[#eefbfd] ${i === activePhoto ? 'border-[#12b8c9]' : 'border-transparent'}`}>
        ...
        <div className="lg:grid lg:grid-cols-3 lg:gap-8">
          <div className="lg:col-span-2 space-y-6 lg:space-y-8">
            {/* Infos hôte */}
            <div className="flex items-start justify-between pb-6 border-b border-[#d5eef2]">
              <div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-full bg-gradient-to-br from-[#12b8c9] to-[#0f2940] flex items-center justify-center text-white font-bold shadow-md">
                  {hostInitial}
                </div>
                <div className="flex flex-col gap-1">
                  {property.superhost && <div className="px-2.5 py-0.5 bg-[#12b8c9] text-white text-xs font-medium rounded-full">Superhost</div>}
            ...
            {/* Points forts */}
            <div className="grid grid-cols-2 gap-3 lg:gap-4 pb-6 border-b border-[#d5eef2]">
            ...
            {/* Équipements */}
            <div className="pb-6 border-b border-[#d5eef2]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {amenitiesList.slice(0, 5).map((amenity, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex items-start gap-3 p-3 lg:p-4 bg-[#eefbfd] rounded-xl">
                      <div className="w-8 h-8 lg:w-10 lg:h-10 bg-[#12b8c9]/20 rounded-lg flex items-center justify-center">
                        <Icon className="w-4 h-4 lg:w-5 lg:h-5 text-[#12b8c9]" />
                      </div>
                ...
                {hasGenerator && (
                  <div className="flex items-start gap-3 p-3 lg:p-4 bg-[#eefbfd] rounded-xl">
                    <div className="w-8 h-8 lg:w-10 lg:h-10 bg-[#12b8c9]/20 rounded-lg flex items-center justify-center">
                      <Zap className="w-4 h-4 lg:w-5 lg:h-5 text-[#12b8c9]" />
                    </div>
                ...
                {hasWaterTank && (
                  <div className="flex items-start gap-3 p-3 lg:p-4 bg-[#eefbfd] rounded-xl">
                    <div className="w-8 h-8 lg:w-10 lg:h-10 bg-[#12b8c9]/20 rounded-lg flex items-center justify-center">
                      <Droplet className="w-4 h-4 lg:w-5 lg:h-5 text-[#12b8c9]" />
                    </div>
            ...
            <div>
              ...
              <div className="bg-gradient-to-br from-[#eefbfd] to-[#e8fffb] rounded-2xl h-48 lg:h-64 flex items-center justify-center border border-[#d5eef2]">
      ...
      {/* Mobile sticky bar */}
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-[#d5eef2] px-4 py-3 flex items-center justify-between z-40">
        <div>
          ...
          <div className="flex items-center gap-1 text-xs text-[#12b8c9]">
            <Star className="w-3 h-3 fill-[#12b8c9]" />
            <span>{property.rating} · {property.reviews} avis</span>
          </div>
        </div>
        <button onClick={onOpenBooking} className="bg-[#12b8c9] text-white px-6 py-3 rounded-full font-medium">Réserver</button>
      </div>
    </div>
  );
}