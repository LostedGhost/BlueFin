      ...
      {VERTICALS.map(({ id, label, icon: Icon }) => {
        ...
        return (
          <button
            key={id}
            onClick={() => onChange(id)}
            className={`flex-shrink-0 flex items-center gap-2 pl-1.5 pr-4 py-1.5 rounded-full border transition-all ${
              isActive
          ...
          >
            ...
            >
              <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-white' : 'text-[#00c9a7]'}`} />
            </span>
